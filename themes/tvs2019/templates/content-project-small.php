...
$skills = trade_get_skills( $post ); /** @see inc/template-tags.php */
...
if($portfolio_config['masonry']){ //check if masonry layout is enabled
	$size = get_post_meta( $id, '_trade_project_featured_image_size', true );
	$thumb_size = 'trade_thumb_' . $size;
	$size_class = 'masonry-'.$size;
}else{
	$thumb_size = 'trade_thumb_' . $portfolio_config['thumb_proportions'];
}
$lightbox_video = "";
$lightbox_img = "";
if($portfolio_config['enable_lightbox'] == "yes"){ //check if lightbox mode is enabled
	$lightbox_video = get_post_meta( $id, '_trade_project_lightbox_video', true );
	$lightbox_img = get_post_meta( $id, '_trade_project_lightbox_img', true );
}
?>
