...
if (!class_exists('trade_gfonts')) {

    class create_gfonts {
	
		// Class Variables
		/* @var $localizationDomain string for translation domain */
        private $localizationDomain 	= 'trade';
        ...
        // function trade_gfonts(){$this->__construct();} // PHP 4
		...
		function tt_gfp_register_customizer_options( $wp_customize ) {
			...
			foreach ($tags_local as $key => $value) {
				...
				array(
					'type'	   => 'text',
					'label'    => __( 'CSS Selectors to Target (optional)', 'trade' ),
		...
		function tt_gfp_generate_styles() {
			...
			foreach($tags_local as $key => $value) {
				...
				// Trade array using the possible font weights for the font. Initialize an empty array if list_font_weights is NULL
