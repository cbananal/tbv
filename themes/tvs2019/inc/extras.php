 ...
 * @return array
 */
function trade_page_menu_args( $args ) {
	$args['show_home'] = true;
	return $args;
}
add_filter( 'wp_page_menu_args', 'trade_page_menu_args' );
 ...
 * @return string id
 */
function trade_get_portfolio_id() {
 ...
 */
function trade_wp_title( $title, $sep ) {
	if ( is_feed() ) {
		return $title;
	}
	...
	if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
		$title .= " $sep " . sprintf( __( 'Page %s', 'trade' ), max( $paged, $page ) );
	}

	return $title;
}
add_filter( 'wp_title', 'trade_wp_title', 10, 2 );
...
add_action( 'wp', 'trade_setup_author' );
...
if( ! function_exists( 'cs_get_bootstrap' ) ) {
  function trade_get_bootstrap( $columns = 1, $device = 'md', $force = false ) {
...
if ( ! function_exists( 'cs_icon_class' ) ) {
  function trade_icon_class( $icon, $before = false ) {
...
if ( ! function_exists( 'trade_mce_buttons' ) ) {
	function trade_mce_buttons( $buttons ) {
		array_unshift( $buttons, 'fontselect' ); // Add Font Select
		array_unshift( $buttons, 'fontsizeselect' ); // Add Font Size Select
		return $buttons;
	}
}
add_filter( 'mce_buttons_2', 'trade_mce_buttons' );

// Customize mce editor font sizes
if ( ! function_exists( 'trade_mce_text_sizes' ) ) {
	function trade_mce_text_sizes( $initArray ){
		$initArray['fontsize_formats'] = "9px 10px 12px 13px 14px 16px 18px 21px 24px 28px 32px 36px";
		return $initArray;
	}
}
add_filter( 'tiny_mce_before_init', 'trade_mce_text_sizes' );
