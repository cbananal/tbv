<?php
/**
 * Custom template tags for Trade.
 * @package create
 */


/**
 * Trade view all link for the shop and assoc. pages
 *
 ...
 */
if ( ! function_exists( 'trade_the_view_all' ) ) {
    function trade_the_view_all( $post_type, $tag = '' ) {
...
if ( ! function_exists( 'trade_the_paging_nav' ) ) {
	function trade_the_paging_nav( $pages = '', $range = 2 ) {
		$showitems = ( $range * 2 ) + 1;
...
if ( ! function_exists( 'trade_the_post_nav' ) ) {
	// Display navigation to next/previous post when applicable.
	function trade_the_post_nav() {
		...
		<nav class="navigation post-navigation" role="navigation">
			<h1 class="screen-reader-text"><?php _e( 'Post navigation', 'trade' ); ?></h1>

			<div class="nav-links clear">
				<?php
				next_post_link( '<div class="nav-next">%link</div>', _x( '<span class="meta-nav">&larr;</span> %title', 'Next post link', 'trade' ) );
				$portfolio_page_id = trade_get_portfolio_id();
				...
				previous_post_link( '<div class="nav-previous">%link</div>', _x( '%title <span class="meta-nav">&rarr;</span>', 'Previous post link', 'trade' ) );
				?>
			</div>
			<div class="clear"></div>
			<!-- .nav-links -->
...
if ( ! function_exists( 'trade_meta_class' ) ) {
	function trade_meta_class() {
		$show_date = get_theme_mod( 'trade_show_meta_date', 'yes' );
		$show_author = get_theme_mod( 'trade_show_meta_author', 'yes' );
		$show_categories = get_theme_mod( 'trade_show_meta_categories', 'yes' );
		$show_comments = get_theme_mod( 'trade_show_meta_comments', 'yes' );
...
if ( ! function_exists( 'trade_the_post_meta' ) ) {
	function trade_the_post_meta() {
		$show_date = get_theme_mod( 'trade_show_meta_date', 'yes' );
		$show_author = get_theme_mod( 'trade_show_meta_author', 'yes' );
		$show_categories = get_theme_mod( 'trade_show_meta_categories', 'yes' );
		$has_meta = false;
		...
		if($show_author == 'yes') {
			$has_meta = true;
			$author = '<span>' . __( 'By ', 'trade' ) . '<a href="'. esc_url(get_author_posts_url( get_the_author_meta( 'ID' ))) .'">'. esc_html( get_the_author() ) .'</a></span>';
			$meta .= $author;
		}
		...
		if($show_categories == 'yes') {
			...
			if ( $categories ) {
				foreach ( $categories as $category ) {
					$output .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . esc_attr( sprintf( __( "View all posts in %s", 'trade' ), $category->name ) ) . '">' . $category->cat_name . '</a>' . $separator;
				}
...
if ( ! function_exists( 'trade_is_blog' ) ) {
	function trade_is_blog() {
		global $post;
...
/**
 * Trade a link of categories without linking to them
 *
 * @param string $id
 */

if ( ! function_exists( 'trade_category_list' ) ) {
	function trade_category_list( $id ) {
...
if ( ! function_exists( 'trade_categorized_blog' ) ) {
	function trade_categorized_blog() {
		if ( false === ( $all_the_cool_cats = get_transient( 'trade_categories' ) ) ) {
			// Trade an array of all the categories that are attached to posts.
			...
			set_transient( 'trade_categories', $all_the_cool_cats );
		}
...
if ( ! function_exists( 'trade_excerpt_ellipsis' ) ) {
	function trade_excerpt_ellipsis( $text ) {
		global $post;

		$create_the_more_link = '&hellip; <a href="' . esc_url( get_permalink() ) . '" title="' . the_title_attribute( 'echo=0' ) . '" class="read-more">';
		$create_the_more_link .= __( 'Read More', 'trade' );
	...
	add_filter( 'the_excerpt', 'trade_excerpt_ellipsis' );
} // if
...
if ( ! function_exists( 'trade_the_more_link' ) ) {
	function trade_the_more_link( $class = '' ) {
		global $post;
...
if( ! function_exists( 'trade_filter_ptags_on_images' ) ) {
	function trade_filter_ptags_on_images( $content ) {
		return preg_replace( '/<p>\s*(<a .*>)?\s*(<img .* \/>)\s*(<\/a>)?\s*<\/p>/iU', '\1\2\3', $content );
	} // create_filter_ptags_on_images

	//add_filter( 'the_content', 'trade_filter_ptags_on_images' );
} // if
...
if( ! function_exists( 'trade_category_transient_flusher' ) ) {
	function trade_category_transient_flusher() {
		// Like, beat it. Dig?
		delete_transient( 'trade_categories' );
	} // create_category_transient_flusher

	add_action( 'edit_category', 'trade_category_transient_flusher' );
	add_action( 'save_post', 'trade_category_transient_flusher' );
} // if
 ...
 * @param int $depth
 */
if( ! function_exists( 'trade_the_comments' ) ) {
	function trade_the_comments( $comment, $args, $depth ) {
		...
		<div class="comment <?php echo get_comment_type(); ?>" id="comment-<?php comment_ID() ?>">
			...
			<?php if ( $comment->comment_approved == '0' ) : ?>
				<p><span class="message"><?php _e( 'Your comment is awaiting moderation.', 'trade' ); ?></span></p>
			<?php endif; ?>
			...
			if ( get_comment_type() != "trackback" )
				comment_reply_link( array_merge( $args, array( 'add_below'  => 'comment',
				                                               'reply_text' => '<span>' . __( 'Reply', 'trade' ) . '</span>',
				                                               'login_text' => '<span>' . __( 'Log in to reply', 'trade' ) . '</span>',
				                                               'depth'      => $depth,
...
if( !function_exists( 'trade_the_pings' ) ) {
	function trade_the_pings( $comment, $args, $depth ) {
		$GLOBALS['comment'] = $comment; ?>
...
if ( ! function_exists( 'trade_clean_and_tag' ) ) {
	function trade_clean_and_tag( $content ) {
 ...
 * @since 1.0
 */
if( ! function_exists( 'trade_filter_nav' ) ) :
	function trade_filter_nav( $post_type = 'project' ) {
		...
		if( sizeof( $categories ) > 0 ) { ?>
			<ul id="filter-nav" class="clearfix">
				<li class="all-btn"><a href="#" data-filter="*" class="selected"><?php _e( 'All', 'trade' ); ?></a></li>
				<?php
 ...
 * @since 1.0
 */
if( ! function_exists( 'trade_get_skills' ) ) :
	function trade_get_skills( $post, $post_type = 'project' ){
 ...
 * @since 1.0
 */
 if( ! function_exists( 'trade_the_thumb_title' ) ) :
	 function trade_the_thumb_title(){
...
if (!function_exists('trade_social_sharing')) {

    function trade_social_sharing() {
					$show_facebook = get_theme_mod( 'trade_show_facebook', 'yes' );
					$show_twitter = get_theme_mod( 'trade_show_twitter', 'yes' );
					$show_google = get_theme_mod( 'trade_show_google', 'yes' );
					$show_linkedin = get_theme_mod( 'trade_show_linkedin', 'yes' );
					$show_pinterest = get_theme_mod( 'trade_show_pinterest', 'yes' );
					$show_tumblr = get_theme_mod( 'trade_show_tumblr', 'yes' );
        			$html = '';
                    ...
                    if ($show_twitter == "yes") {
                        ...
                        $html .= '<a href="#" title="' . __("Share on Twitter", 'trade') . '" onclick="popUp=window.open(\'http://twitter.com/home?status=' . urlencode( get_permalink() ) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
