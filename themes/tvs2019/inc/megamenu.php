...
class Trade_Megamenu extends TT_Abstract{
  ...
  public function __construct() {
    ...
    $this->addAction( 'trade_mega_menu_fields', 'trade_mega_menu_fields', 10, 2 );
    $this->addAction( 'trade_mega_menu_labels', 'trade_mega_menu_labels' );
   ...
   *
   */
  public function trade_mega_menu_fields( $item_id, $item ) {
  ?>

  <div class="field-icon description description-wide">
    <?php
      $hidden = ( empty( $item->icon ) ) ? ' hidden' : '';
      $icon   = ( !empty( $item->icon ) ) ? ' class="'. trade_icon_class( $item->icon ) . '"' : '';
    ?>
    <div class="create_field create_field_icon">
      <div class="icon-select">
        <span class="button icon-picker <?php echo $item->icon; ?>"><?php _e(" Icon", 'trade'); ?></span>
  ...
  <div class="mega-menu">
    ...
    <div class="field-mega-width">
      <select id="edit-menu-item-mega_width-<?php echo $item_id; ?>" name="menu-item-mega_width[<?php echo $item_id; ?>]" class="is-width">
        <option value="full-width"><?php _e("Full Width", 'trade'); ?></option>
        <?php
    ...
    <div class="mega-depend-width hidden">
      <p class="description">
        <label for="edit-menu-item-mega_custom_width<?php echo $item_id; ?>">
          <?php _e("Custom Width (without px)", 'trade'); ?><br />
  ...
  <div class="">
    ...
    <p class="field-column description description-thin">
      <label for="edit-menu-item-column-title-<?php echo $item_id; ?>">
        <input type="checkbox" id="edit-menu-item-column-title-<?php echo $item_id; ?>" value="1" name="menu-item-column_title[<?php echo $item_id; ?>]"<?php checked( $item->column_title, 1 ); ?> /> <?php _e("Disable Title Text", 'trade'); ?>
      </label>
    </p>

    <p class="field-column description description-thin last">
      <label for="edit-menu-item-column-title-link-<?php echo $item_id; ?>">
        <input type="checkbox" id="edit-menu-item-column-title-link-<?php echo $item_id; ?>" value="1" name="menu-item-column_title_link[<?php echo $item_id; ?>]"<?php checked( $item->column_title_link, 1 ); ?> /> <?php _e("Disable Link", 'trade'); ?>
      </label>
    </p>
    <div class="clear"></div>
  </div>
  ...
  public function trade_mega_menu_labels() {
