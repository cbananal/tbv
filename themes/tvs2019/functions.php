...
if ( ! function_exists( 'trade_setup' ) ) :
	function trade_setup() {
		...
		add_image_size( 'trade_thumb_square', 700, 700, true );
		add_image_size( 'trade_thumb_landscape', 800, 600, true );
		add_image_size( 'trade_thumb_portrait', 600, 800, true );
		add_image_size( 'trade_thumb_wide', 1000, 500, true );
		add_image_size( 'trade_thumb_tall', 500, 1000, true );
		add_image_size( 'trade_thumb_wide_tall', 1000, 1000, true );

		// 1.2.5 This theme uses wp_nav_menu() in one location.
		register_nav_menus( array(
			'primary' => __( 'Main Menu', 'trade' ),
			'slide_panel' => __( 'Slide Panel Desktop', 'trade' ),
			'slide_panel_mobile' => __( 'Slide Panel Mobile', 'trade' ),
			'left' => __( 'Left Menu (Split Header)', 'trade' )
		) );

		// 1.2.6 Enable support for Custom Backgrounds
		// only enabled if layout mode is boxed.
		if(get_theme_mod( 'trade_site_width' ) == "boxed"){
		...
		$portfolio_cpt = new Portfolio_CPT( 'trade' ); // Sending in the textdomain
		...
		$testimonial_cpt = new Testimonial_CPT( 'trade' ); // Sending in the textdomain
		...
		load_template( get_template_directory() . '/inc/page-builder-widgets/typed-text-widget/typed-text-widget.php', true );
		...
		function trade_body_class($classes) {
		        ...
		        $site_container = get_theme_mod( 'trade_site_width', '' );
				$header_position = 'top-header';
				$header_layout = get_theme_mod( 'trade_header_top_layout', 'inline-header' );
				$header_sticky = get_theme_mod( 'trade_sticky_header', '' );
		...
		add_filter('body_class', 'trade_body_class');



		function trade_post_class( $classes ) {
			global $post;
			$full_width_content = get_post_meta( $post->ID, '_trade_post_full_width', true );
			if($full_width_content=='yes'){
				$classes[] = "full";
			}
			return $classes;
		}
		add_filter( 'post_class', 'trade_post_class' );
...
add_filter( 'document_title_separator', 'trade_document_title_separator',10 );
function trade_document_title_separator( $sep ){
	return '|';
}
 ...
 * Loader Animation
 */
if( !function_exists( 'trade_loader' ) ){
function trade_loader(){
	$loader_enabled = get_theme_mod( 'trade_loader_enabled', 'no' );
	if($loader_enabled == 'yes'){
	$loader_animation = get_theme_mod( 'trade_loader_animation', 'loading' );
	echo '<div id="loader-container">';
	echo '<div id="loader-icon" class="'.$loader_animation.'"></div>';
 ...
 * Scroll to top
 */
if( !function_exists( 'trade_scroll_to_top' ) ){
function trade_scroll_to_top(){
	$scroll_to_top_enabled = get_theme_mod( 'trade_enable_header_scroll_to_top', 'yes' );
 ...
 */
if( !function_exists( 'trade_get_paged_corrected' ) ){
function trade_get_paged_corrected(){
	global $paged;
	if ( get_query_var('paged') ) { $paged = get_query_var('paged'); }
	elseif ( get_query_var('page') ) { $paged = get_query_var('page'); }
	else { $paged = 1; }
}
add_action( 'pre_get_posts', 'trade_get_paged_corrected' );
} //if()
 ...
 */
if( !function_exists( 'trade_small_class' ) ){
function trade_small_class( $classes ) {
    ...
    add_filter( 'post_class', 'trade_small_class' );
} //if()
...
// 2.1 Trade Scripts
if ( ! function_exists( 'trade_scripts_n_styles' ) ) {
	function trade_scripts_n_styles() {

		// 2.1.1  CSS
		wp_enqueue_style( 'trade-style', get_stylesheet_uri(), false, 5 );
		wp_enqueue_style( 'trade-owl-carousel', get_template_directory_uri() . '/css/owl.carousel.css', false, '1.3.3', 'all' );
		wp_enqueue_style( 'trade-owl-theme', get_template_directory_uri() . '/css/owl.theme.css', false, '1.3.3', 'all' );
		wp_enqueue_style( 'trade-prettyphoto', get_template_directory_uri() . '/css/prettyPhoto.css', false, '3.1.6', 'all' );
		wp_enqueue_style( 'superfish', get_template_directory_uri() . '/css/superfish.css', false, '1.7.5', 'all' );
		wp_enqueue_style( 'trade-owl-theme', get_template_directory_uri() . '/css/owl.theme.css', false, '1.3.3', 'all' );
		wp_enqueue_style( 'trade-woocommerce', get_template_directory_uri() . '/css/woocommerce.css', false, '1.0', 'all' );

		// 2.1.2  Fonts
		wp_enqueue_style( 'trade-font-awesome', get_template_directory_uri() . '/css/font-awesome.min.css', false, '4.0.3', 'all' );

		// 2.1.3  Scripts
		wp_enqueue_script( 'trade-jquery-actual', get_template_directory_uri().'/js/jquery.actual.js', array( 'jquery' ), '1.0.16', true );
		...
		wp_enqueue_script( 'trade-wait-for-images', get_bloginfo( 'template_url' ) . '/js/jquery.waitforimages.min.js', array( 'jquery' ), '2.0.2', true );
		wp_register_script( 'trade-parallax-scroll', get_bloginfo( 'template_url' ) . '/js/jquery.parallax-scroll.js', array( 'jquery' ), '0.6.30', true );
		wp_register_script( 'trade-hover-3d', get_bloginfo( 'template_url' ) . '/js/jquery.hover3d.js', array( 'jquery' ), '1.0', true );
		wp_enqueue_script( 'trade-owl', get_bloginfo( 'template_url' ) . '/js/owl.carousel.min.js', array( 'jquery' ), '1.3.3', true );
		wp_enqueue_script( 'trade-isotope', get_bloginfo( 'template_url' ) . '/js/jquery.isotope.js', array( 'jquery' ), '1.5.25', true );
		wp_enqueue_script( 'trade-fitvids', get_template_directory_uri() . '/js/jquery.fitvids.js', array( 'jquery' ), '1.0', true );
		wp_enqueue_script( 'trade-imagesloaded', get_template_directory_uri() . '/js/imagesloaded.js', array('jquery'), '3.1.6', false);
		wp_enqueue_script( 'trade-scrollto', get_template_directory_uri() . '/js/jquery.scrollTo.js', array( 'jquery' ), '1.4.6', true );
		wp_enqueue_script( 'trade-prettyphoto', get_template_directory_uri() . '/js/jquery.prettyPhoto.js', array( 'jquery' ), '3.1.6', true );
		wp_enqueue_script( 'trade-theme_trust_js', get_template_directory_uri() . '/js/theme_trust.js', array( 'jquery' ), '2.0', true );
...
add_action( 'wp_enqueue_scripts', 'trade_scripts_n_styles', 10000000000 );


//	2.2 Customizer Head
if ( ! function_exists( 'trade_theme_head' ) ) {
	function trade_theme_head() { ?>
		<?php if (get_theme_mod('trade_favicon') ) : ?>
			<link rel="shortcut icon" href="<?php echo get_theme_mod('trade_favicon'); ?>" />
		<?php endif; ?>
		...
		//Color
		$color = array();
		$color['accent'] 		    = get_theme_mod( 'trade_accent_color' );
		$color['base_text'] 		= get_theme_mod( 'trade_base_text_color' );
		$color['link'] 			    = get_theme_mod( 'trade_link_color' );
		$color['link_hover'] 	    = get_theme_mod( 'trade_link_hover_color' );
		$color['button'] 			= get_theme_mod( 'trade_button_color' );
		$color['button_text'] 		= get_theme_mod( 'trade_button_text_color' );
		$color['header_bkg'] 	    = get_theme_mod( 'trade_header_bkg_color' );
		$color['header_sticky_bkg'] = get_theme_mod( 'trade_sticky_header_bkg_color' );
		$color['header_accent'] 	= get_theme_mod( 'trade_header_accent_color' );
		$color['main_menu']         = get_theme_mod( 'trade_main_menu_color' );
		$color['main_menu_hover']   = get_theme_mod( 'trade_main_menu_hover_color' );
		$color['sticky_main_menu']  = get_theme_mod( 'trade_sticky_main_menu_color' );
		$color['sticky_main_menu_hover'] = get_theme_mod( 'trade_sticky_main_menu_hover_color' );
		$color['site_title'] = get_theme_mod( 'trade_site_title_color' );
		$color['sticky_site_title'] = get_theme_mod( 'trade_sticky_site_title_color' );
		$color['loader'] = get_theme_mod( 'trade_loader_color' );
		$color['loader_bkg'] = get_theme_mod( 'trade_loader_bkg_color' );
		$color['drop_down_bg'] = get_theme_mod( 'trade_drop_down_bg_color' );
		$color['drop_down_link'] = get_theme_mod( 'trade_drop_down_link_color' );
		$color['drop_down_link_hover'] = get_theme_mod( 'trade_drop_down_link_hover_color' );
		$color['drop_down_divider'] = get_theme_mod( 'trade_drop_down_divider_color' );
		$color['slide_panel_bg'] = get_theme_mod( 'trade_slide_panel_bg_color' );
		$color['slide_panel_text'] = get_theme_mod( 'trade_slide_panel_text_color' );
		$color['slide_panel_link'] = get_theme_mod( 'trade_slide_panel_link_color' );
		$color['slide_panel_link_hover'] = get_theme_mod( 'trade_slide_panel_link_hover_color' );
		$color['slide_panel_divider'] = get_theme_mod( 'trade_slide_panel_divider_color' );
		$color['scroll_to_top_bg'] = get_theme_mod( 'trade_scroll_to_top_bg_color' );
		$color['scroll_to_top_arrow'] = get_theme_mod( 'trade_scroll_to_top_arrow_color' );
		$color['footer_bg'] = get_theme_mod( 'trade_footer_bg_color' );
		$color['footer_widget_title'] = get_theme_mod( 'trade_footer_widget_title_color' );
		$color['footer_text'] = get_theme_mod( 'trade_footer_text_color' );
		$color['footer_link'] = get_theme_mod( 'trade_footer_link_color' );
		$color['footer_link_hover'] = get_theme_mod( 'trade_footer_link_hover_color' );
		$color['product_hover'] = get_theme_mod( 'trade_product_hover_color' );
		$color['shop_accent'] = get_theme_mod( 'trade_shop_accent_color' );
		$color['page_title_text_color'] = get_theme_mod( 'trade_page_title_text_color' );
		$color['page_title_bg_color'] = get_theme_mod( 'trade_page_title_bg_color' );

		//Logo
		$logo_width = array();
		$logo_width['top'] = get_theme_mod( 'trade_logo_top_width' );
		$logo_width['top_light'] = get_theme_mod( 'trade_logo_top_width_light' );
		$logo_width['sticky'] = get_theme_mod( 'trade_logo_sticky_width' );
		$logo_width['side'] = get_theme_mod( 'trade_logo_side_width' );
		$logo_width['mobile_max'] = get_theme_mod( 'trade_logo_mobile_width' );

		//Header
		$header = array();
		$header['top_header_height'] = intval(get_theme_mod( 'trade_top_header_height', 90 ));
		$header['sticky_header_height'] = intval(get_theme_mod( 'trade_sticky_header_height', 60 ));
		$header['mobile_breakpoint'] = intval(get_theme_mod( 'trade_mobile_header_breakpoint' ));

		//Page Titles
		$page_title = array();
		$page_title['alignment'] = get_theme_mod( 'trade_page_title_alignment' );
		...
		if(get_post_meta( $id, '_trade_header_menu_color', true )){
			$color['main_menu_metabox'] = get_post_meta( $id, '_trade_header_menu_color', true );
		}
		if(get_post_meta( $id, '_trade_header_menu_hover_color', true )){
			$color['main_menu_hover_metabox'] = get_post_meta( $id, '_trade_header_menu_hover_color', true );
		}

		//Title
		$title = array();
		$title['height'] = intval(get_post_meta( $id, '_trade_title_area_height', true ));
		$title['color'] = get_post_meta( $id, '_trade_title_color', true );
		$title['bg_img'] = get_post_meta( $id, '_trade_title_bg_img', true );
		...
		<style>
			...
			<?php // Loader Color
			if( $color['loader'] ) { ?>
				<?php $rgb = hex2rgba($color['loader']); ?>
				<?php $rgba = hex2rgba($color['loader'], .2); ?>
				.loading { 
				...
				@-webkit-keyframes pulse {
				  50% {
				    background: <?php echo $color['loader']; ?>;
				  }
				}
				@keyframes pulse {
				  50% {
			...
			if( $color['slide_panel_text'] ) { ?>
				...
				#slide-panel .close:before, #slide-panel .close:after {	background-color: <?php echo $color['slide_panel_text']; ?>;
				}
		...
		if( $header ){ ?>
			...
			if( $header['top_header_height'] ) { ?>
				...
				#site-header.main .main-nav #menu-main-menu > li > span { line-height: <?php echo $header['top_header_height']; ?>px; height: <?php echo $header['top_header_height']; ?>px;}
			<?php } ?>
			...
			if( $header['sticky_header_height'] ) { ?>
				...
				#site-header.sticky .hamburger { height: <?php echo $header['sticky_header_height']; ?>px !important; }
				#site-header.sticky .nav-holder,
				#site-header.sticky .main-nav ul > li,
				#site-header.sticky .main-nav ul > li > a,
				#site-header.sticky .main-nav ul > li > span { line-height: <?php echo $header['sticky_header_height']; ?>px; height: <?php echo $header['sticky_header_height']; ?>px;}
			<?php } ?>
		...
		<?php if (get_theme_mod('trade_custom_css') ) {
			echo get_theme_mod('trade_custom_css');
		} ?>
...
add_action( 'wp_head','trade_theme_head' );


// 2.3 Admin Scripts and Styles
if ( ! function_exists( 'trade_admin_styles' ) ) {
	function trade_admin_styles() {
		wp_enqueue_style( 'trade-font-awesome', get_template_directory_uri() . '/css/font-awesome.min.css', false, '4.0.3', 'all' );
		wp_enqueue_style( 'trade-admin-css', get_template_directory_uri() . '/css/admin.css');
		wp_enqueue_style( 'trade-icon-picker-css', get_template_directory_uri() . '/css/icon-picker.css');
		wp_enqueue_script( 'trade-admin-js', get_bloginfo( 'template_url' ) . '/js/admin.js', array( 'jquery' ), '1.0', true );
		wp_enqueue_script( 'trade-icon-picker', get_bloginfo( 'template_url' ) . '/js/icon-picker.js', array( 'jquery' ), '1.0', true );
	}
}
add_action('admin_enqueue_scripts', 'trade_admin_styles');
...
function trade_fonts_url() {
	 ...
	 * into your own language.
	 */
	$open_sans = _x( 'on', 'Open Sans font: on or off', 'trade' );

	$roboto = _x( 'on', 'Roboto font: on or off', 'trade' );

	if ( 'off' !== $roboto || 'off' !== $open_sans) {
	    $font_families = array();

	    if ( 'off' !== $roboto ) {
	        $font_families[] = 'Roboto:300,400,500,700,300italic,400italic,700italic';
	    }
...
function trade_gfonts() {

    wp_enqueue_style( 'trade-fonts', trade_fonts_url(), array(), null );

}
add_action( 'wp_enqueue_scripts', 'trade_gfonts' );
...
function ocdi_import_files() {
    return array(
        array(
            'import_file_name'             => 'Trade Demo Content',
            'local_import_file'            => trailingslashit( get_template_directory() ) . '/inc/demo-import/demo-content/trade-content.xml',
            'local_import_widget_file'     => trailingslashit( get_template_directory() ) . '/inc/demo-import/demo-content/trade-widgets.json',
            'local_import_customizer_file' => '',
            'import_preview_image_url'     => '',
            'import_notice'                => __( '', 'trade' ),
        ),
    );
}
...
function ocdi_before_widgets_import( $selected_import ) {
	...
	// Set Front Page
	$front_page = get_page_by_title('Finance');
	...
	// Import Slider
	$_FILES["import_file"]["tmp_name"] = get_template_directory().'/inc/demo-import/demo-content/medical.zip';
	$slider = new RevSlider();
	$response = $slider->importSliderFromPost();
	unset($slider);
	
	// Import Slider
	$_FILES["import_file"]["tmp_name"] = get_template_directory().'/inc/demo-import/demo-content/fitness.zip';
	$slider = new RevSlider();
	$response = $slider->importSliderFromPost();
	unset($slider);
 ...
 */
add_filter( 'wpforms_shareasale_id', 'tttrade_wpforms_shareasale_id' );
function tttrade_wpforms_shareasale_id( $shareasale_id ) {
